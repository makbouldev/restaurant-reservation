...
export default function CreativeNavbar({ currentRoute, lang, setLang, t, logoPath, logoSize, brandName, brandNameVisible, restaurantName, restaurantInfo }) {
  ...
  const primaryPhone = String(restaurantInfo?.phone || '').trim() || '+212 522 49 16 16';
  const secondaryPhone = String(restaurantInfo?.whatsappPhone || '').trim() || '+212 522 20 71 11';
  const phoneHref = primaryPhone.replace(/\s+/g, '');
  const secondaryPhoneHref = secondaryPhone.replace(/\s+/g, '');
  ...
  }, [menuOpen, servicesOpen, currentRoute, lang]);

  return (
    <nav className="creative-nav" ref={navRef}>
      <div className="container">
        <div className="creative-nav-shell">
          <div className="creative-nav-top-row">
            ...
            <div className="creative-nav-phones">
              <a href={`tel:${phoneHref}`} className="creative-nav-phone">
                <span aria-hidden="true">&#9742;</span>
                {primaryPhone}
              </a>
              <a href={`tel:${secondaryPhoneHref}`} className="creative-nav-phone">
                <span aria-hidden="true">&#9742;</span>
                {secondaryPhone}
              </a>
            </div>
